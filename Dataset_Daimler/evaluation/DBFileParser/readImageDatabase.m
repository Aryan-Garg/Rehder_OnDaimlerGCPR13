...
while (~feof(fid))
    ...
    elseif (strcmp(token,obj_2d_separator)) 
        tmpobj =  read2DObject(fid);
        idb.objects(objCount) = tmpobj; 
        idb.images(imgCount-1).objList(imgObjIndex) = objCount;
        objCount = objCount+1;
        imgObjIndex = imgObjIndex+1;
    elseif (strcmp(token,comment_attribute))
        [key value] = readAttribute(fid);
        idb.objects(objCount-1).attributes.(key) = value;
    else 
        error(['Parse error: Unknown token ''' token '''' ' followed by line ''' fgetl(fid) '''' 'in file: ' filename ]);
    end
end


if (~isfield(idb,'objects'))
    idb.objects=struct([]);
end

% add fields to store 3d detection area information
idb.detection_area_x =  [];
idb.detection_area_y =  [];
idb.detection_area_z =  [];
